...
def run_experiment(seed: int, use_auto_stop: bool) -> None:
    ...
    result_path = f"results/acrobot/{result_name}_seed_{seed}.csv"
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        print(f"\nRunning fixed-budget PPO on Acrobot, seed {seed}")
        run_experiment(seed=seed, use_auto_stop=False)

    for seed in seeds:
        print(f"\nRunning automatic-stopping PPO on Acrobot, seed {seed}")
        run_experiment(seed=seed, use_auto_stop=True)

    print("\nFinished. Acrobot results are saved in results/acrobot/")